...
create or replace function public.sincronizar_cartera_contrato_v114()returns trigger language plpgsql security definer set search_path=''as $fn$
begin insert into public.contrato_cartera_v114(contrato_id,fecha_vencimiento_base,actualizado_por)values(new.id,coalesce(new.fecha_entrega,(new.fecha_ingreso at time zone'America/Guayaquil')::date),new.actualizado_por)on conflict(contrato_id)do nothing;return new;end;$fn$;
...
create or replace function public.obtener_cartera_contrato_v114(p_contrato_id uuid)returns jsonb language plpgsql stable security definer set search_path=''as $fn$
declare v_r jsonb;
begin if auth.uid()is null then raise exception'Debes iniciar sesion';end if;if not public.usuario_tiene_permiso_v35('contratos.cartera.ver')then raise exception'No tienes permiso para consultar cartera';end if;
 ...
 if v_r is null then raise exception'El contrato no existe';end if;return v_r;end;$fn$;

create or replace function public.configurar_cartera_contrato_v114(p_contrato_id uuid,p_fecha_vencimiento date,p_nota text,p_motivo text,p_idempotency_key uuid)returns jsonb language plpgsql security definer set search_path=''as $fn$
...
begin if v_uid is null then raise exception'Debes iniciar sesion';end if;if not public.usuario_tiene_permiso_v35('contratos.cartera.editar')then raise exception'No tienes permiso para gestionar cartera';end if;if p_idempotency_key is null then raise exception'La idempotencia es obligatoria';end if;select datos into v_r from public.cliente_eventos_v113 where idempotency_key=p_idempotency_key;if found then return v_r;end if;if p_fecha_vencimiento is null then raise exception'Elige una fecha de vencimiento';end if;if length(btrim(coalesce(p_motivo,'')))<10 then raise exception'El motivo debe tener al menos 10 caracteres';end if;
 ...
 v_r:=jsonb_build_object('contrato_id',p_contrato_id,'fecha_vencimiento',p_fecha_vencimiento);insert into public.cliente_eventos_v113(cliente_id,accion,datos,motivo,usuario_id,idempotency_key)select cliente_id_v113,'configurar_cartera',v_r,btrim(p_motivo),v_uid,p_idempotency_key from public.contratos where id=p_contrato_id;return v_r;end;$fn$;

create or replace function public.registrar_promesa_pago_v114(p_contrato_id uuid,p_fecha_prometida date,p_monto numeric,p_nota text,p_idempotency_key uuid)returns jsonb language plpgsql security definer set search_path=''as $fn$
...
begin if v_uid is null then raise exception'Debes iniciar sesion';end if;if not public.usuario_tiene_permiso_v35('contratos.cartera.editar')then raise exception'No tienes permiso para registrar abonos';end if;if p_idempotency_key is null then raise exception'La idempotencia es obligatoria';end if;if length(btrim(coalesce(p_motivo,'')))<10 then raise exception'El motivo debe tener al menos 10 caracteres';end if;if p_fecha is null or p_fecha>(now()at time zone'America/Guayaquil')::date then raise exception'La fecha del abono no es valida';end if;if coalesce(p_monto,0)<=0 then raise exception'El monto debe ser mayor que cero';end if;if p_medio_pago not in('efectivo','transferencia','tarjeta','cheque','otro')then raise exception'El medio de pago no es valido';end if;if p_medio_pago in('transferencia','tarjeta','cheque')and length(btrim(coalesce(p_referencia,'')))<3 then raise exception'Este medio de pago exige referencia';end if;
 ...
 insert into public.contrato_finanzas_eventos_v100(contrato_id,abono_id,accion,valor_anterior,valor_nuevo,motivo,usuario_id,idempotency_key,resultado)values(p_contrato_id,v_id,'registrar_abono',v_c.abono,v_total,btrim(p_motivo),v_uid,p_idempotency_key,v_r);if v_total>=v_c.presupuesto then update public.contrato_promesas_pago_v114 set estado='cumplida',resuelto_at=now(),motivo_resolucion='Saldo cancelado'where contrato_id=p_contrato_id and estado='vigente';end if;return v_r;end;$fn$;

create or replace function public.registrar_recordatorio_cobro_v114(p_contrato_id uuid,p_canal text,p_resultado text,p_proximo_recordatorio date,p_idempotency_key uuid)returns jsonb language plpgsql security definer set search_path=''as $fn$
