...
create or replace function public.catalogo_ingreso_contrato_v115()returns jsonb language plpgsql stable security definer set search_path=''as $v115$
declare v_r jsonb;
begin if auth.uid()is null then raise exception'Debes iniciar sesion';end if;if not public.usuario_tiene_permiso_v35('contratos.editar')then raise exception'No tienes permiso para ingresar contratos';end if;
 select jsonb_build_object('almacenes',coalesce(jsonb_agg(jsonb_build_object('id',a.id,'nombre',a.nombre,'codigo',a.codigo)order by a.nombre)filter(where a.id is not null),'[]'))into v_r from public.almacenes a where a.activo and public.usuario_puede_almacen(a.id,false);return v_r;end;$v115$;

create or replace function public.crear_contrato_v115(p_datos jsonb,p_idempotency_key uuid)returns jsonb language plpgsql security definer set search_path=''as $v115$
...
create or replace function public.consolidado_comercial_v115(p_desde date,p_hasta date,p_vendedores text[]default null,p_almacenes uuid[]default null,p_canales text[]default null)returns jsonb language plpgsql stable security definer set search_path=''as $v115$
...
begin if auth.uid()is null then raise exception'Debes iniciar sesion';end if;if not public.usuario_tiene_permiso_v35('reportes.comercial.ver')then raise exception'No tienes permiso para consultar el consolidado comercial';end if;if v_hasta<v_desde or v_hasta-v_desde>730 then raise exception'El rango debe estar entre 1 y 731 dias';end if;
 ...
 'contratos',coalesce((select jsonb_agg(jsonb_build_object('id',id,'numero',numero,'nombre_contrato',nombre_contrato,'cliente',cliente,'cliente_confirmado',cliente_confirmado_v115,'vendedor',vendedor,'tienda',tienda,'canal',canal,'fecha',fecha,'entrega',fecha_entrega,'prendas',total_prendas,'facturado',presupuesto,'cobrado',abono,'saldo',saldo,'entregado',entregado,'entrega_a_tiempo',entrega_a_tiempo)order by fecha desc,numero desc)from base),'[]'))into v_r;return v_r;end;$v115$;

create or replace function public.guardar_comision_vendedor_v115(p_vendedor text,p_porcentaje numeric,p_base text,p_vigente_desde date,p_vigente_hasta date,p_nota text,p_motivo text,p_idempotency_key uuid)returns jsonb language plpgsql security definer set search_path=''as $v115$
