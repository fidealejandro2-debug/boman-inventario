...
export default function BriefPublicoCliente({token}:{token:string}){
 ...
 return <main style={{maxWidth:1180,margin:"0 auto",padding:"24px 16px 60px"}}>
   <header className={estilos.cabecera}><div><BomanLogo className={estilos.logo}/><h1>Brief de producción</h1><p>Documento compartido de Boman Sport.</p></div>{datos&&<button className={estilos.noImprimir} onClick={()=>window.print()}>Imprimir / guardar PDF</button>}</header>
