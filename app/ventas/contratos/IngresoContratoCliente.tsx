...
export default function IngresoContratoCliente({perfil,editarId}:{perfil:Perfil;editarId?:string}){
 ...
 const [vendedores,setVendedores]=useState<VendedorUsuario[]>([]);
 const [vendedorBloqueado,setVendedorBloqueado]=useState(false);
 const diferenciasTallasAceptadas=useRef("");
 const [borradorListo,setBorradorListo]=useState(false);
 const [estadoBorrador,setEstadoBorrador]=useState("Preparando borrador…");
 const [firmaGuardada,setFirmaGuardada]=useState<string|null>(null);
 const [visitados,setVisitados]=useState<number[]>([0]);
 const recuperarRef=useRef<Promise<boolean>|null>(null);
 const formularioRef=useRef<HTMLElement>(null);
 const firmaActual=JSON.stringify(form);
 const archivosPendientes=form.archivos.some(archivo=>archivo.file&&!archivo.url);
 useProteccionSalida(!resultado && (editando ? firmaGuardada!==null && firmaGuardada!==firmaActual : archivosPendientes || estadoBorrador==="Guardando borrador…" || estadoBorrador==="No se pudo guardar el borrador en este dispositivo."));
 useEffect(()=>{setVisitados(actual=>actual.includes(paso)?actual:[...actual,paso]);formularioRef.current?.focus();},[paso]);
 const total=useMemo(()=>form.prendas.reduce((s,x)=>s+Math.max(0,Number(x.cantidad)||0),0),[form.prendas]);
 const saldo=Math.max(0,Number(form.cab.presupuesto||0)-(editando?Number(form.cab.abono||0):0));
 ...
 useEffect(()=>{
  if(editando){setBorradorListo(true);return}
  let vivo=true;
  void (async()=>{
   try{
    const raw=localStorage.getItem(BORRADOR);
    if(raw){const d=JSON.parse(raw) as Form;if(d?.cab&&Array.isArray(d.prendas)){
     recuperarRef.current??=confirmarDialogo("Hay un borrador guardado en este dispositivo. ¿Deseas recuperarlo?");
     const recuperar=await recuperarRef.current;if(!vivo)return;
     if(recuperar)setForm({...d,cab:{...cabInicial(perfil.nombre_completo),...d.cab},archivos:(d.archivos||[]).filter(x=>x.url)});
     else localStorage.removeItem(BORRADOR);
    }}
   }catch{if(vivo)setEstadoBorrador("No se pudo recuperar el borrador anterior.")}
   finally{if(vivo)setBorradorListo(true)}
  })();return()=>{vivo=false};
 },[perfil.nombre_completo,editando]);
 ...
 useEffect(()=>{if(!editarId)return;let vivo=true;setCargandoEdicion(true);
  void supabase.rpc("obtener_expediente_contrato_v97",{p_contrato_id:editarId}).then(({data,error})=>{
   ...
   try{const cargado=formDesdeContrato(data as never);cargado.prendas=expandirLineas(cargado.lineas);setFirmaGuardada(JSON.stringify(cargado));setForm(cargado)}
 ...
 useEffect(()=>{void supabase.rpc("catalogo_ingreso_contrato_v147").then(({data,error})=>{if(error)return void mostrarAvisoDialogo(error.message,"No se pudo cargar el catálogo comercial",true);const catalogo=data as{almacenes?:AlmacenVenta[];vendedores?:VendedorUsuario[];vendedor_bloqueado?:boolean};const lista=catalogo.vendedores||[];setAlmacenesVenta(catalogo.almacenes||[]);setVendedores(lista);setVendedorBloqueado(Boolean(catalogo.vendedor_bloqueado));if(lista.length===1)setForm(f=>({...f,cab:{...f.cab,vendedor_perfil_id:lista[0].id,vendedor:lista[0].nombre,vendedor_responsable:lista[0].nombre}}))})},[supabase]);
 useEffect(()=>{
  if(editando||!borradorListo||resultado)return;
  setEstadoBorrador("Guardando borrador…");
  const t=setTimeout(()=>{
   try{const seguro={...form,archivos:form.archivos.filter(x=>x.url).map(({file,preview,...x})=>x)};localStorage.setItem(BORRADOR,JSON.stringify(seguro));setEstadoBorrador("Borrador guardado en este dispositivo.")}
   catch{setEstadoBorrador("No se pudo guardar el borrador en este dispositivo.")}
  },900);return()=>clearTimeout(t);
 },[form,editando,borradorListo,resultado]);
 ...
 function errorPaso(n=paso){const c=form.cab;if(n===0&&(!c.vendedor_perfil_id||!texto(c.vendedor)||!texto(c.nombre_contrato_v115)||!texto(c.cliente)||!texto(c.canal)||!texto(c.telefono)))return"Selecciona el usuario vendedor y completa canal, contrato, cliente y teléfono.";if(n===0&&almacenesVenta.length>1&&!c.almacen_venta_id_v115)return"Selecciona la tienda o local donde se realizó la venta.";if(n===1&&(!c.fecha_entrega||!c.tipo_contrato||!c.prioridad))return"Completa tipo, prioridad y fecha de entrega.";if(n===2&&!form.prendasSel.length)return"Selecciona al menos una prenda del pedido.";if(n===5&&(!form.prendas.length||total<1))return"Agrega al menos una línea de talla con cantidad.";if(n===4&&(!c.nombre_tecnica||!c.numero_tecnica||!c.sellos_tpu))return"Completa las técnicas de nombre, número y TPU.";if(n===6&&(!texto(c.vendedor_responsable)||!c.autorizado))return"Confirma el vendedor responsable y la autorización de producción.";if(n===6){const r=comprobarFacturacion(form.prendas,form.facturacion);if(r.hayPrendas&&!r.ok)return`La facturación no cuadra con las prendas. ${r.faltan.length?"Faltan: "+r.faltan.join(", ")+". ":""}${r.sobran.length?"Sobran: "+r.sobran.join(", ")+".":""}`;}return""}
 ...
 return <>
  <header className={estilos.cabecera}><div><span className="eyebrow">{editando?"PRODUCCIÓN · REEDICIÓN":"VENTAS"}</span><h1>{editando?"Editar contenido del contrato":"Ingreso de contratos"}</h1><p>{editando?"Prendas, tallas, jugadores, especificaciones y mockups. El número, el estado y las cifras no se tocan desde aquí.":"Pedido, brief técnico, tallas, diseños y valores conectados directamente con producción."}</p></div><button className="secondary" onClick={()=>setPreview(true)}>Vista previa</button></header>
  <nav className={estilos.pasos} aria-label="Pasos del contrato">{PASOS.map((x,i)=>{const completo=visitados.includes(i)&&!errorPaso(i);return <button type="button" key={x} disabled={guardando} aria-current={i===paso?"step":undefined} aria-label={x+(i===paso?", paso actual":completo?", revisado":", pendiente de revisar")} className={i===paso?estilos.activo:completo?estilos.completo:""} onClick={()=>void abrirPaso(i)}><b>{i!==paso&&completo?"✓":i+1}</b><span>{x}</span></button>})}</nav>
  <dl className="contract-summary" aria-label="Resumen del contrato"><div><dt>Cliente</dt><dd>{form.cab.cliente||"Por completar"}</dd></div><div><dt>Prendas</dt><dd>{total}</dd></div><div><dt>Entrega</dt><dd>{form.cab.fecha_entrega||"Por definir"}</dd></div><div><dt>Presupuesto</dt><dd>{Number(form.cab.presupuesto||0).toLocaleString("es-EC",{style:"currency",currency:"USD"})}</dd></div><div className="draft-status" role="status">{editando ? firmaActual!==firmaGuardada?"Cambios pendientes de guardar.":"Sin cambios pendientes." : estadoBorrador}{archivosPendientes&&" Hay archivos que se subirán al registrar el contrato."}</div></dl>
  <section ref={formularioRef} tabIndex={-1} aria-label={"Paso "+(paso+1)+": "+PASOS[paso]} className={`card ${estilos.formulario}`}>
   {paso===0&&<PasoCliente supabase={supabase} form={form} setCab={setCab} almacenes={almacenesVenta} vendedores={vendedores} vendedorBloqueado={vendedorBloqueado} busqueda={busqueda} setBusqueda={setBusqueda} buscar={buscarAnterior} buscando={buscando} coincidencias={coincidencias} cargar={cargarReposicion}/>}
   ...
   {paso===6&&<PasoCierre form={form} setCab={setCab} saldo={saldo} editando={editando} agregar={agregarFact} cambiar={cambiar} quitar={quitar}/>}
   <footer className={estilos.acciones}><button className="secondary" disabled={paso===0||guardando} onClick={()=>setPaso(x=>x-1)}>Anterior</button><span>Paso {paso+1} de 7</span>{paso<6?<button onClick={()=>void irSiguiente()}>Continuar</button>:<><button className="secondary" onClick={()=>setPreview(true)}>Revisar</button><button onClick={()=>void guardar()} disabled={guardando}>{guardando?"Subiendo y guardando…":editando?"Guardar cambios":"Registrar contrato"}</button></>}</footer>
  </section>
...
function PasoCliente({supabase,form,setCab,almacenes,vendedores,vendedorBloqueado,busqueda,setBusqueda,buscar,buscando,coincidencias,cargar}:{supabase:ReturnType<typeof createClient>;form:Form;setCab:SetCab;almacenes:AlmacenVenta[];vendedores:VendedorUsuario[];vendedorBloqueado:boolean;busqueda:string;setBusqueda:(x:string)=>void;buscar:()=>void;buscando:boolean;coincidencias:any[];cargar:(id:string)=>void}){function elegirVendedor(id:string){const v=vendedores.find(x=>x.id===id);setCab("vendedor_perfil_id",id);if(v){setCab("vendedor",v.nombre);setCab("vendedor_responsable",v.nombre)}}return <><h2>Vendedor, contrato y cliente</h2><div className={estilos.reposicion}><div><strong>¿Es una reposición?</strong><span>Copia el brief anterior y crea un contrato nuevo.</span></div><div><input value={busqueda} onChange={e=>setBusqueda(e.target.value)} onKeyDown={e=>e.key==="Enter"&&buscar()} placeholder="Cliente, contrato o BOM-2026-…"/><button className="secondary" onClick={buscar}>{buscando?"Buscando…":"Buscar"}</button></div>{coincidencias.map(x=><button className={estilos.resultado} key={x.id} onClick={()=>cargar(x.id)}><strong>{x.numero}</strong><span>{x.cliente} · {x.total_prendas} prendas</span></button>)}</div><div className={estilos.grid}><Campo titulo="Usuario vendedor *"><><select value={form.cab.vendedor_perfil_id} disabled={vendedorBloqueado} onChange={e=>elegirVendedor(e.target.value)}><option value="">Seleccionar vendedor…</option>{vendedores.map(v=><option key={v.id} value={v.id}>{v.nombre}</option>)}</select><small className={estilos.pista}>{vendedorBloqueado?"Este contrato quedará vinculado a tu usuario.":"La propiedad define quién verá el contrato en su panel."}</small></></Campo><Campo titulo="Canal de venta *"><select value={form.cab.canal} onChange={e=>setCab("canal",e.target.value)}><option value="">Seleccionar…</option>{CANALES.map(x=><option key={x}>{x}</option>)}</select></Campo><Campo titulo="Nombre del contrato *"><><input value={form.cab.nombre_contrato_v115} onChange={e=>setCab("nombre_contrato_v115",e.target.value)} placeholder="Ej. Uniformes Club Los Andes"/><small className={estilos.pista}>Así se identificará el pedido y aparecerá en el brief.</small></></Campo><ClienteReal supabase={supabase} valor={form.cab.cliente} setCab={setCab}/>{almacenes.length>0&&<Campo titulo={`Tienda / local${almacenes.length>1?" *":""}`}><select value={form.cab.almacen_venta_id_v115} onChange={e=>setCab("almacen_venta_id_v115",e.target.value)}><option value="">{almacenes.length===1?`Automática: ${almacenes[0].nombre}`:"Seleccionar…"}</option>{almacenes.map(a=><option key={a.id} value={a.id}>{a.nombre}</option>)}</select></Campo>}<Campo titulo="Teléfono *"><input value={form.cab.telefono} onChange={e=>setCab("telefono",e.target.value)} placeholder="0999123456"/></Campo><Campo titulo="WhatsApp"><input value={form.cab.whatsapp} onChange={e=>setCab("whatsapp",e.target.value)}/></Campo><Campo titulo="Correo"><input type="email" value={form.cab.email} onChange={e=>setCab("email",e.target.value)}/></Campo></div></>}
...
function PasoCierre({form,setCab,saldo,editando,agregar,cambiar,quitar}:{form:Form;setCab:SetCab;saldo:number;editando:boolean;agregar:()=>void;cambiar:Cambiar;quitar:Quitar}){return <>
 <h2>Producción, valores y cierre</h2>
 <div className={estilos.grid}>
  <Campo titulo="Presupuesto total (USD)"><input type="number" min={0} step=".01" value={form.cab.presupuesto} onChange={e=>setCab("presupuesto",Number(e.target.value))}/></Campo>
  {editando&&<Campo titulo="Abonado registrado"><input type="number" value={form.cab.abono} disabled/></Campo>}
  {!editando&&<Campo titulo="Registro de cobros"><small className={estilos.pista}>El contrato inicia sin abono. Registra cada pago desde Panel de vendedores indicando la fecha real, efectivo o transferencia y su referencia.</small></Campo>}
  <div className={estilos.saldo}><span>Saldo pendiente</span><strong>${saldo.toFixed(2)}</strong></div>
  <Campo titulo="Forma de entrega"><select value={form.cab.forma_entrega} onChange={e=>setCab("forma_entrega",e.target.value)}><option value="">Seleccionar…</option>{FORMAS_ENTREGA.map(x=><option key={x}>{x}</option>)}</select></Campo>
  <Campo titulo="Dirección"><input value={form.cab.direccion} onChange={e=>setCab("direccion",e.target.value)}/></Campo>
  <Campo titulo="Instrucciones especiales" ancho><textarea rows={4} value={form.cab.instrucciones} onChange={e=>setCab("instrucciones",e.target.value)}/></Campo>
  <Campo titulo="Vendedor responsable *"><input value={form.cab.vendedor_responsable} disabled/></Campo>
 </div>
 <Titulo titulo="Detalle de facturación" texto="Cómo se factura el pedido (ej. 20 Uniforme completo + 10 Camiseta). Debe cuadrar con las prendas del contrato para poder guardar." accion={agregar} etiqueta="+ Agregar línea de facturación"/>
 <ComprobanteFact form={form}/>
 {form.facturacion.map(x=><div className={estilos.filaFact} key={x.id}><select value={x.concepto} onChange={e=>cambiar<Fact>("facturacion",x.id,{concepto:e.target.value})}>{CONCEPTOS_FACT.map(o=><option key={o.l} value={o.l}>{o.l}{o.c?"":" (no valida)"}</option>)}</select><select value={x.calidad} onChange={e=>cambiar<Fact>("facturacion",x.id,{calidad:e.target.value})}>{CALIDADES.map(x=><option key={x}>{x}</option>)}</select><input type="number" min={1} value={x.cantidad} onChange={e=>cambiar<Fact>("facturacion",x.id,{cantidad:Number(e.target.value)})}/><label className={estilos.check}><input type="checkbox" checked={x.obsequio} onChange={e=>cambiar<Fact>("facturacion",x.id,{obsequio:e.target.checked})}/> Obsequio</label><button className="secondary" onClick={()=>quitar("facturacion",x.id)}>Quitar</button></div>)}
 <label className={estilos.autoriza}><input type="checkbox" checked={form.cab.autorizado} onChange={e=>setCab("autorizado",e.target.checked)}/><span><strong>Confirmo que revisé todos los datos.</strong> Autorizo el envío de este contrato a producción.</span></label>
 </>}
