...
export default function IngresoContratoCliente({perfil}:{perfil:Perfil}){
 ...
 return <>
  ...
  <section className={`card ${estilos.formulario}`}>
   ...
   {paso===1&&<PasoContrato form={form} setCab={setCab} supabase={supabase} total={total}/>}
...
function PasoContrato({form,setCab,supabase,total}:{form:Form;setCab:SetCab;supabase:ReturnType<typeof createClient>;total:number}){return <><h2>Tipo de contrato y fechas</h2><div className={estilos.tipos}>{[["Normal","600 prendas/día"],["Equipo Profesional","100 prendas/día"],["Mercadería","150 prendas/día"],["Emergente","50 prendas/día"]].map(x=><button className={form.cab.tipo_contrato===x[0]?estilos.seleccionado:""} onClick={()=>setCab("tipo_contrato",x[0])} key={x[0]}><strong>{x[0]}</strong><small>{x[1]}</small></button>)}</div><div className={estilos.grid}><Campo titulo="Fecha deseada por el cliente *"><input type="date" value={form.cab.fecha_entrega} onChange={e=>{const v=e.target.value;setCab("fecha_entrega",v);if(v)setCab("fecha_inicio_produccion",habilesAntes(v,5))}}/></Campo><Campo titulo="Inicio de producción"><><input type="date" value={form.cab.fecha_inicio_produccion} onChange={e=>setCab("fecha_inicio_produccion",e.target.value)}/><small className={estilos.pista}>Auto: 5 días laborables antes de la fecha deseada. Puedes ajustarlo manualmente.</small></></Campo><Campo titulo="Prioridad *"><select value={form.cab.prioridad} onChange={e=>setCab("prioridad",e.target.value)}><option>Normal</option><option>Urgente</option></select></Campo></div><CapacidadDia supabase={supabase} fecha={form.cab.fecha_inicio_produccion} tipo={form.cab.tipo_contrato} total={total}/><p className={estilos.notaLili}>⚠️ Nota: Lili asignará la fecha exacta de inicio de producción según la capacidad del taller.</p></>}
