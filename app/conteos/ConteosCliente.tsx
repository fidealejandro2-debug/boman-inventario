  ...
  return (
    <>
      ...
      {activo && <section className="card conteo-activo">
        ...
        <BuscadorCodigoProducto compacto etiqueta="Escanear producto contado" onEncontrado={async (producto) => { if (!activo.lineas.some((linea) => linea.producto_id === producto.producto_id)) { await mostrarAvisoDialogo("El producto leído no forma parte de este conteo.", "Producto fuera del conteo"); return; } setBusquedaConteo(producto.sku); setCategoriaConteo(""); setTimeout(() => document.querySelector<HTMLInputElement>(`input[data-conteo-producto="${producto.producto_id}"]`)?.focus(), 0); }} />
        ...
        <div className="tabla-scroll"><table><thead><tr><th>SKU</th><th>Producto</th><th>Categoría</th><th>Talla</th><th className="num">Cantidad física</th><th>Observación</th></tr></thead><tbody>{lineasActivasFiltradas.map((l) => <tr key={l.id}><td><strong>{l.producto?.sku}</strong></td><td>{l.producto?.nombre}</td><td>{l.producto?.categoria ?? "Sin categoría"}</td><td>{l.producto?.talla ?? "-"}</td><td className="num"><input data-conteo-producto={l.producto_id} type="number" min={0} value={valores[l.producto_id] ?? ""} onChange={(e) => setValores({ ...valores, [l.producto_id]: e.target.value })} style={{ width: 90, textAlign: "right" }} /></td><td><input value={observaciones[l.producto_id] ?? ""} onChange={(e) => setObservaciones({ ...observaciones, [l.producto_id]: e.target.value })} /></td></tr>)}{!lineasActivasFiltradas.length && <tr><td colSpan={6} className="vacio">No hay productos que coincidan con la búsqueda y categoría.</td></tr>}</tbody></table></div>
