...
export async function POST(request: NextRequest) {
  ...
  const registrarIntento = (exitoso:boolean, mensaje:string|null) => admin.rpc("registrar_intento_respaldo_v135", {
    p_ingreso_id: ingreso.id, p_exitoso: exitoso, p_error: mensaje,
  });
  const base=process.env.BOMANSPORT_WEBAPP_URL,token=process.env.BOMANSPORT_API_TOKEN;
  if(!base||!token){const mensaje="Faltan BOMANSPORT_WEBAPP_URL o BOMANSPORT_API_TOKEN";await registrarIntento(false,mensaje);return NextResponse.json({ok:false,pendiente:true,error:mensaje},{status:503})}
  ...
  try{
    ...
    await registrarIntento(true,null);
    return NextResponse.json({ok:true,url:remoto.url||null});
  }catch(e){const mensaje=e instanceof Error?e.message:"No se pudo respaldar";await registrarIntento(false,mensaje);return NextResponse.json({ok:false,pendiente:true,error:mensaje},{status:502})}
}
