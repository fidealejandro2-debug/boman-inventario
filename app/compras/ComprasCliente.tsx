...
export default function ComprasCliente({ perfil }: { perfil: Perfil }) {
  ...
  return <>
    <header className="page-heading workspace-heading"><div><span className="eyebrow">COMPRAS</span><h1>Compras multiempresa</h1><p>Proveedor compartido, RUC comprador, aprobación, recepción parcial y cuarentena.</p></div><div className="acciones-documento"><Link className="btn-enlace secondary" href="/compras/importar-xml">Importar XML</Link>{puedeCrear && tab === "ordenes" && <button onClick={() => setMostrarOrden((v) => !v)}>{mostrarOrden ? "Cancelar" : "+ Nueva orden"}</button>}{puedeGestionarProveedor && tab === "proveedores" && <button onClick={() => editarProveedor()}>+ Nuevo proveedor</button>}</div></header>
    {msg && <div className={msg.tipo === "error" ? "error" : "success"}>{msg.texto}</div>}
    <div className="tabs workflow-nav" aria-label="Áreas de compras"><button className={`tab ${tab === "ordenes" ? "activo" : ""}`} onClick={() => setTab("ordenes")}>Órdenes de compra</button><button className={`tab ${tab === "proveedores" ? "activo" : ""}`} onClick={() => setTab("proveedores")}>Proveedores ({proveedores.filter((p) => p.activo).length})</button></div>
