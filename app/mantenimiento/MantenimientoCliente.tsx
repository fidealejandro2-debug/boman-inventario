...
export default function MantenimientoCliente({ perfil }: { perfil: Perfil }) {
  ...
  const [fotosActivo, setFotosActivo] = useState<Activo | null>(null);
  ...
  return <section className="mantenimiento-centro">
    ...
    {tab === "activos" && <div className="card"><div className="tabla-scroll"><table><thead><tr><th>Activo</th><th>Empresa / ubicación</th><th>Estado</th><th>Criticidad</th><th>Lectura</th><th>Próximo mantenimiento</th><th>Órdenes</th><th>Fotos</th>{puedeEditar && <th>Acciones</th>}</tr></thead><tbody>
    ...
    {fotosActivo && <div className="modal-operativo" onMouseDown={(e) => { if (e.target === e.currentTarget) setFotosActivo(null); }}><div className="modal-contenido ancho"><div className="header-row"><div><h2>{fotosActivo.codigo} · {fotosActivo.nombre}</h2><p className="conteo">Fotos generales, placa, estado y detalles del activo.</p></div><button className="secondary" onClick={() => setFotosActivo(null)}>Cerrar</button></div><GaleriaImagenes entidadTipo="activo" entidadId={fotosActivo.id} titulo={fotosActivo.nombre} puedeEditar={puedeEditar} /></div></div>}
  </section>;
}
