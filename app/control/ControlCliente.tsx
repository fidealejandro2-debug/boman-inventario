...
export default function ControlCliente({ perfil }: { perfil: Perfil }) {
  ...
  async function cargar() {
    ...
    const [d, e, p, inc] = await Promise.all([
      ...
      `).neq("estado", "resuelta").order("fecha_limite").limit(100),
    ]);
  ...
  const unidadesDiferencia = useMemo(() => incidencias.reduce((total, inc) => total + inc.lineas.reduce(
  ...
  return (
    <>
      ...
      <div className="kpis">
        ...
        <div className={`kpi ${incidencias.length ? "alerta" : "ok"}`}><div className="label">No conformidades abiertas</div><div className="valor">{incidencias.length}</div><small>{unidadesDiferencia} unidad(es) bajo seguimiento</small></div>
