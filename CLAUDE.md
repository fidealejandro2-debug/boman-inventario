...
**Los códigos incorrectos del facturador se conservan y se concilian; no se disfrazan de códigos legales.** `empresa_equivalencias_facturacion` puede relacionar, por ejemplo, el XML temporal `001-006` con el establecimiento oficial Puyo `006` y el punto oficial `100`. El XML mantiene su número original, la carga exige confirmación visible y `documentos_venta_xml` registra quién confirmó la novedad, cuándo y por qué. Un código desconocido puede aplicarse después de validación manual para no detener la operación, pero queda marcado como no estándar.
